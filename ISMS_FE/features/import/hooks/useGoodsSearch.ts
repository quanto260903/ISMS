// ============================================================
//  features/inward/hooks/useGoodsSearch.ts
//  Tìm kiếm hàng hóa + portal dropdown (giống sale)
// ============================================================

"use client";

import { useState, useRef, useEffect, useCallback } from "react";
import { searchGoods } from "../import.api";
import type { GoodsSearchResult, DropdownState, DropdownPos, InwardItem } from "../types/import.types";

interface Props {
  updateItem: (index: number, field: keyof InwardItem, value: unknown) => void;
  onSelectGoods: (index: number, goods: GoodsSearchResult) => void;
  onAddItem: () => void;
}

export function useGoodsSearch({ updateItem, onSelectGoods, onAddItem }: Props) {
  ...
  const openPortalDropdown = useCallback((index: number) => {
    ...
    setDropdownPos({ top: rect.bottom + 4, left: rect.left, width: Math.max(rect.width, 480), index });
  }, []);
    ...
    const handler = (e: MouseEvent) => {
      dropdownRefs.current.forEach((ref, i) => {
        if (ref && !ref.contains(e.target as Node))
          setDropdowns((prev) => prev.map((d, idx) => idx === i ? { ...d, open: false } : d));
      });
      setDropdownPos((prev) => {
        if (!prev) return null;
  ...
  const handleGoodsIdChange = useCallback(
    (index: number, value: string, totalItems: number) => {
      ...
      setDropdowns((prev) => prev.map((d, i) => i === index ? { ...d, loading: true, open: true } : d));
      ...
      debounceTimers.current[index] = setTimeout(async () => {
        try {
          ...
          setDropdowns((prev) =>
            prev.map((d, i) => i === index ? { suggestions: results, loading: false, open: true } : d)
          );
          openPortalDropdown(index);
        } catch {
          setDropdowns((prev) =>
  ...
  const handleInputFocus = useCallback(
    (index: number, e: React.FocusEvent<HTMLInputElement>) => {
      ...
      if (dd?.suggestions.length > 0) {
        setDropdowns((prev) => prev.map((d, i) => i === index ? { ...d, open: true } : d));
        openPortalDropdown(index);
      }
    },
