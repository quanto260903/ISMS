// ============================================================
//  features/sale/components/SaleListPage.tsx
//  Danh sách phiếu bán hàng + tìm kiếm, lọc, phân trang
// ============================================================
    ...
    <div style={s.page}>
      ...
      <div style={s.heroBanner}>
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          <div style={s.heroEyebrow}>WMS Pro · Quản lý bán hàng</div>
          <h1 style={s.heroTitle}>Danh sách phiếu bán hàng</h1>
          <p style={s.heroSub}>Quản lý toàn bộ phiếu bán, tra cứu và theo dõi tình trạng đơn hàng</p>
        </div>
        <button style={s.btnPrimary} onClick={() => router.push("/dashboard/sale/new")}>
          ...
          <div style={{ ...s.filterGroup, flex: 1 }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              <input
                ...
                placeholder="Nhập số phiếu, tên khách hàng..."
        ...
        <table style={s.table}>
          <thead>
            <tr style={s.theadRow}>
              ...
              <th style={s.th}>Ngày bán</th>
              <th style={s.th}>Khách hàng</th>
              <th style={{ ...s.th, textAlign: "right" }}>Số mặt hàng</th>
          ...
          <tbody>
            {loading && (
              <tr>
                <td colSpan={7} style={s.statusCell}>
              ...
              <tr>
                <td colSpan={7} style={s.statusCell}>
                  <div style={s.emptyState}>
                    ...
                    <p style={{ fontWeight: 600, color: "#374151" }}>Chưa có phiếu bán nào</p>
                    <p style={{ fontSize: 12, color: "#94a3b8", marginTop: 4 }}>
                      Nhấn "Tạo phiếu bán mới" để bắt đầu
                    </p>
                  </div>
                </td>
              </tr>
            ...
            {!loading && result?.items.map((row, i) => (
              <SaleRow
                ...
                row={row}
                i={i}
                onView={() => router.push(`/dashboard/sale/${row.voucherId}?mode=view`)}
              />
            ))}
          </tbody>
        </table>
...
function SaleRow({
  row, i, onView,
}: { row: SaleListItem; i: number; onView: () => void }) {
  const [hover, setHover] = React.useState(false);

  return (
    <tr
      style={{
        background: hover ? "#fdf4ff" : i % 2 === 0 ? "#fff" : "#fafafa",
    ...
    >
      ...
      <td style={{ ...s.td, textAlign: "right", color: "#64748b" }}>{row.itemCount}</td>
...
const s: Record<string, React.CSSProperties> = {
  ...
  btnRefresh:  { height: 38, width: 38, border: "1.5px solid #e2e8f0", borderRadius: 8, background: "#fdf4ff", cursor: "pointer", fontSize: 20, display: "flex", alignItems: "center", justifyContent: "center", color: "#7c3aed", marginTop: 22 },
  ...
  spinnerRing: { width: 36, height: 36, borderRadius: "50%", border: "3px solid #f3e8ff", borderTopColor: "#7c3aed" },
  ...
  grandTotalAmount: { fontSize: 17, fontWeight: 800, color: "#7c3aed", letterSpacing: "-0.3px" },
  pagination:  { display: "flex", alignItems: "center", gap: 6 },
  pageInfo:    { fontSize: 13, color: "#64748b", marginRight: 6 },
  pageBtn:     { width: 32, height: 32, border: "1.5px solid #e2e8f0", borderRadius: 7, background: "#fdf4ff", cursor: "pointer", fontSize: 13, display: "flex", alignItems: "center", justifyContent: "center", color: "#7c3aed", fontWeight: 700 },
  ...
  btnView:     { padding: "5px 14px", border: "1.5px solid #e9d5ff", borderRadius: 7, background: "#fdf4ff", color: "#7c3aed", fontWeight: 600, fontSize: 12, cursor: "pointer" },
};
